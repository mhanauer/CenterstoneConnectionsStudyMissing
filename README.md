...
```
Get rid of not important variables first
LivingWhere.x ~ Employment.x+ Depression.x + HealthStatus.x + Gender.x + EducationYears.x + Age.x + DAUseIllegDrugsDays.x + County.x 
...
test = glmer(LivingWhere.x ~ Employment.x+ Depression.x + HealthStatus.x + Gender.x + EducationYears.x + Age.x + DAUseIllegDrugsDays.x + County.x + (1 | ClientID), data  = PHQ9_GAD7Long, family = "binomial")
...
summary(PHQ9_GAD7LongMissingDesOut)
...
round(results,3) 
```
Try interaction effects with county*depression
```{r}
head(PHQ9_GAD7LongCenter[[1]])
...
for(i in 1:m){
  output[[i]] = glmer(LivingWhere.x ~ Employment.x+ Depression.x + HealthStatus.x + Gender.x + EducationYears.x + Age.x + DAUseIllegDrugsDays.x + County.x*Depression.x + (1 | ClientID), data  = PHQ9_GAD7LongCenter[[i]], family = "binomial")
  rSquared[[i]] = r.squaredGLMM(output[[i]])
  output[[i]] = summary(output[[i]])
  coef_output[[i]] = output[[i]]$coefficients[,1]
  se_output[[i]] = output[[i]]$coefficients[,2]
...
round(results,3) 
```
Try interaction effects with County.x*HealthStatus.x
```{r}
head(PHQ9_GAD7LongCenter[[1]])
...
for(i in 1:m){
  output[[i]] = glmer(LivingWhere.x ~ Employment.x+ Depression.x + HealthStatus.x*County.x + Gender.x + EducationYears.x + Age.x + DAUseIllegDrugsDays.x + Depression.x + (1 | ClientID), data  = PHQ9_GAD7LongCenter[[i]], family = "binomial")
  rSquared[[i]] = r.squaredGLMM(output[[i]])
  output[[i]] = summary(output[[i]])
  coef_output[[i]] = output[[i]]$coefficients[,1]
  se_output[[i]] = output[[i]]$coefficients[,2]
...
round(results,3) 
```
Try interaction effects with County.x*DAUseIllegDrugsDays.x
```{r}
head(PHQ9_GAD7LongCenter[[1]])
